...
// is wrong is RENDERING it: a buyer greeted by "Y***t" is the visible half of
// the same bug that let this template ship invented Spanish personas instead.
//
...
// plausible Spanish name and not "Cliente verificado": CanonicalReview carries
// `{ text, rating, author, dateRaw, variant }` and NO verification signal at
// all, so the fallback states only what is true — someone wrote this.
//
// THE LIMIT OF THIS HELPER, STATED SO NOBODY RELIES ON IT WRONGLY. It cannot
// detect a FABRICATED author, only a masked one: "Marina Sosto" is perfectly
// readable and would be preserved verbatim. That is correct behaviour for a
// real source name, which is why the invented personas had to be removed from
// src/data/testimonials.ts at the DATA level instead. Masking is a render
// decision; fabrication is a provenance defect and is fixed at the origin.
...
/**
 * The name to render. Readable authors are preserved verbatim; masked or
 * absent ones become `Cliente`.
 */

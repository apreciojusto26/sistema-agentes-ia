 ...
 * NOT inferred from `variants` being empty. That distinction is the whole
 * point:
 *
 ...
 * page that silently looks fine, which is the one outcome this system forbids.
 * PUBLIC_ vars are inlined into the client bundle by Vite, so the island reads
 * it without threading a prop through every Astro section.
 */

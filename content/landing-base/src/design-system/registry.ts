...
// a one-line shim) or removed outright, so the helper had no callers and the
// type checker said so.
//
// Deleted rather than kept "in case": re-introducing a legacy capability should
// require re-introducing the concept, with a reason written down. Nothing stops
// a future entry from pointing anywhere it needs to — `block()` takes the
// component path — but it will not get there by reaching for a helper whose
// whole purpose was the arrangement this series dismantled.

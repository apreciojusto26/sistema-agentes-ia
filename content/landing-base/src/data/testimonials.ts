import type { Testimonial } from '@/types/content';

/**
 * Real customer reviews for AstraVibe (star projector), lightly polished from
 * the AliExpress listing. Covered by the footer disclaimer ("Sitio de demostración...").
 */
export const testimonials: Testimonial[] = [
  {
    id: 't-featured',
    author: 'Marina Sosto',
    location: 'Valencia',
    rating: 5,
    date: '2026-05-22',
    title: 'La atmósfera es muy fuerte',
    body: 'Estoy muy satisfecho. Solo cuando la habitación está completamente oscura se logra el efecto completo, y la atmósfera que crea es muy fuerte. Los chicos duermen mucho mejor.',
    verified: true,
    variant: 'quote',
  },
  {
    id: 't-reel-1',
    author: 'Julián Ferreiro',
    location: 'Madrid',
    rating: 5,
    date: '2026-11-16',
    body: 'La proyección es clara y delicada, y la textura de la vía láctea es muy realista. Me resulta especialmente útil para dormir y relajarme después del trabajo.',
    verified: true,
    variant: 'reel',
  },
  {
    id: 't-reel-2',
    author: 'Carla Bettini',
    location: 'Sevilla',
    rating: 5,
    date: '2026-05-22',
    body: 'Muy claro y fácil de usar, me gusta mucho. Lo usa mi hija todas las noches como luz de dormir.',
    verified: true,
    variant: 'reel',
  },
  {
    id: 't-reel-3',
    author: 'Pablo Iriarte',
    location: 'Bilbao',
    rating: 5,
    date: '2026-05-12',
    body: 'Es hermoso y facilísimo de usar. Se enciende con un toque y en un segundo tenés el cielo estrellado en el techo.',
    verified: true,
    variant: 'reel',
  },
  {
    id: 't-reel-4',
    author: 'Noelia Campos',
    location: 'Zaragoza',
    rating: 5,
    date: '2026-05-27',
    body: 'Lo compré como regalo para un amigo y le gusta muchísimo. Proyección nítida y entrega rápida.',
    verified: true,
    variant: 'reel',
  },
  {
    id: 't-reel-5',
    author: 'Diego Marconi',
    location: 'Málaga',
    rating: 5,
    date: '2026-05-30',
    body: 'Es hermoso, tal cual lo muestran las fotos. Ojo con elegir bien la opción: la cantidad de películas varía según la variante que elijas.',
    verified: true,
    variant: 'reel',
  },
  {
    id: 't-reel-6',
    author: 'Sofía Aranda',
    location: 'Alicante',
    rating: 5,
    date: '2026-06-13',
    body: 'Una buena luz nocturna con proyector. Lo mejor es que el enfoque se puede ajustar para que se vea nítido en cualquier distancia.',
    verified: true,
    variant: 'reel',
  },
  {
    id: 't-card-1',
    author: 'Lucía Prado',
    location: 'Vigo',
    rating: 5,
    date: '2026-06-29',
    body: 'Llegó en perfecto estado y muy bien embalado. Tiene un aspecto fantástico y la calidad es buena.',
    verified: true,
    variant: 'card',
  },
  {
    id: 't-card-2',
    author: 'Martín Ocampo',
    location: 'Córdoba',
    rating: 5,
    date: '2026-07-20',
    body: 'Increíble. Llegó rapidísimo, se ve de muy alta calidad y además tiene función de luz nocturna. Excelente compra.',
    verified: true,
    variant: 'card',
  },
  {
    id: 't-card-3',
    author: 'Rocío Ibáñez',
    location: 'Granada',
    rating: 5,
    date: '2026-06-20',
    body: 'Me siento como un astronauta. El efecto espacial en el techo es espectacular, lo recomiendo al 100%.',
    verified: true,
    variant: 'card',
  },
];
...
/**
 * Removes one GENERATOR-OWNED key's line, if present. writeEnvKey()'s
 * counterpart: a key this script writes when its input IS present must also
 * disappear when that input is ABSENT on a later --force regeneration.
 *
 * FOUND FOR REAL, not theoretical: a Commerce run passed `siteUrl: null`
 * against an output folder an EARLIER, unrelated run had left with a real
 * SITE_URL — the .env's own key survived because nothing ever removed it,
 * `astro.config.mjs`'s configuredSite() found it via `siteUrlFromEnvFile()`,
 * og:image rendered anyway, and Grammar V4's capability check correctly
 * failed the mismatch it exists to catch. "Absent" must mean absent in the
 * regenerated output, never "whatever an earlier run left there".
 *
 * SCOPED STRICTLY to keys this script itself writes elsewhere in this file
 * (SITE_URL, PUBLIC_SHOPIFY_PRODUCT_HANDLE) — never called against the three
 * credentials or anything else an operator added by hand. Leaves every other
 * line, including the commented credential placeholders, untouched.
 */
function deleteEnvKey(envPath, key) {
  if (!existsSync(envPath)) return;
  const pattern = new RegExp(`^${escapeRegExp(key)}=.*\\n?`, 'm');
  const previous = readFileSync(envPath, 'utf-8');
  if (!pattern.test(previous)) return;
  writeFileSync(envPath, previous.replace(pattern, ''));
}
...
async function main() {
  ...
  await withStage('todos', () => {
    ...
    if (args.siteUrl) {
      ...
      // REPRODUCIBILITY: this run's own input is what the output must
      // represent — never a PREVIOUS run's SITE_URL surviving a --force
      // regeneration that asked for none. See deleteEnvKey()'s own header
      // for the real incident this closes.
      deleteEnvKey(path.join(outDir, '.env'), 'SITE_URL');
    }
    ...
    if (args.shopifyHandle) {
      ...
      // REPRODUCIBILITY, same rule as SITE_URL above: a Commerce→Preview
      // regeneration of the SAME output must not leave an earlier run's
      // product handle behind. catalog.ts never reads it in preview mode
      // (resolveCommerceMode() short-circuits first), so this was inert at
      // runtime — but a stale value sitting in the file is still not what
      // this run's own inputs said to produce.
      deleteEnvKey(path.join(outDir, '.env'), 'PUBLIC_SHOPIFY_PRODUCT_HANDLE');

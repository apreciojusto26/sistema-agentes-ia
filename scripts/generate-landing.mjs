  ...
  withStage('todos', () => {
    ...
    if (args.shopifyHandle) {
      ...
      if (existsSync(envPath)) {
        ...
        const handleLine = `PUBLIC_SHOPIFY_PRODUCT_HANDLE=${args.shopifyHandle}`;
        const updated = /^PUBLIC_SHOPIFY_PRODUCT_HANDLE=.*$/m.test(previous)
          ? previous.replace(/^PUBLIC_SHOPIFY_PRODUCT_HANDLE=.*$/m, handleLine)
          : `${previous.replace(/\n*$/, '\n')}${handleLine}\n`;
        writeFileSync(envPath, updated);
        console.log(`✓ commerce mode — PUBLIC_SHOPIFY_PRODUCT_HANDLE=${args.shopifyHandle} updated in the existing .env (other keys preserved)`);
      } else {
        writeFileSync(
          envPath,
          [
